...
template<class T> struct ScalarFromVec { typedef typename T type; };
template<class T, int P> struct ScalarFromVec<Dual<T,P>> { typedef Dual<typename T,P> type; };
...
make_Vec3 (const Dual<Vec3,P> &x, const Dual<Vec3,P> &y, const Dual<Vec3,P> &z)
{
    Dual<Vec3,P> result;
...
template<int P>
OSL_HOSTDEVICE inline Dual<Vec3,P>
make_Vec3 (const Dual<Vec3,P> &x, const Dual<Vec3,P> &y)
{
    Dual<Vec3,P> result;
    OSL_INDEX_LOOP(i, P+1, {
...
make_Color3 (const Dual<Color3,P> &x, const Dual<Color3,P> &y, const Dual<Color3,P> &z)
{
...
make_Vec2 (const Dual<Vec2,P> &x, const Dual<Vec2,P> &y)
{
    Dual<Vec2,P> result;
    OSL_INDEX_LOOP(i, P+1, {
...
template<int P>
OSL_HOSTDEVICE OSL_FORCEINLINE OSL_CONSTEXPR14 Dual<Vec3, P>
...
//template<int P>
//OSL_HOSTDEVICE OSL_FORCEINLINE OSL_CONSTEXPR14 Dual<Color3,P>
...
template<int P>
OSL_HOSTDEVICE OSL_FORCEINLINE OSL_CONSTEXPR14 Dual<Vec2,P>
            ...
            Dual<Vec3,P> &dst)
{
    ...
    Dual<Vec3,P> src0 = comp_x(src), src1 = comp_y(src), src2 = comp_z(src);
    Dual<Vec3,P> a = src0 * M.x[0][0] + src1 * M.x[1][0] + src2 * M.x[2][0];
    Dual<Vec3,P> b = src0 * M.x[0][1] + src1 * M.x[1][1] + src2 * M.x[2][1];
    Dual<Vec3,P> c = src0 * M.x[0][2] + src1 * M.x[1][2] + src2 * M.x[2][2];
    dst = make_Vec3 (a, b, c);
}
...
operator* (const Dual<Vec3,P> &src, const Imath::Matrix33<T> &M)
{
    ...
    // Vec<Dual>, do the usual matrix math, then reshuffle again.
    Dual<Vec3,P> src0 = comp_x(src), src1 = comp_y(src), src2 = comp_z(src);
    Dual<Vec3,P> a = src0 * M[0][0] + src1 * M[1][0] + src2 * M[2][0];
    Dual<Vec3,P> b = src0 * M[0][1] + src1 * M[1][1] + src2 * M[2][1];
    Dual<Vec3,P> c = src0 * M[0][2] + src1 * M[1][2] + src2 * M[2][2];
...
//    // The simplest way to express this is to break up the Dual<Vec> into
//    // Vec<Dual>, do the usual matrix math, then reshuffle again.
//    Dual<Color3,P> src0 = comp_x(src), src1 = comp_y(src), src2 = comp_z(src);
//    Dual<Color3,P> a = src0 * M[0][0] + src1 * M[1][0] + src2 * M[2][0];
//    Dual<Color3,P> b = src0 * M[0][1] + src1 * M[1][1] + src2 * M[2][1];
//    Dual<Color3,P> c = src0 * M[0][2] + src1 * M[1][2] + src2 * M[2][2];
...
robust_multVecMatrix(const Imath::Matrix44<S>& M, const Vec3& src, Vec3& dst)
{
    ...
    if (OSL_LIKELY(! equalVal (w, Vec3(0)))) {
        dst.x = a / w;
        dst.y = b / w;
        dst.z = c / w;
    } else {
                      ...
                      const Dual<Vec3,P> &in, Dual<Vec3,P> &out)
{
    ...
    Imath::Vec3<Dual<Vec3,P>> din(comp_x(in), comp_y(in), comp_z(in)), dout;
...
template<int P>
OSL_HOSTDEVICE inline OSL_CONSTEXPR14 Dual<Vec3, P>
...
template<int P>
OSL_HOSTDEVICE inline OSL_CONSTEXPR14 Dual<Vec2,P>
...
normalize (const Dual<Vec3,P> &a)
{
    ...
    if (OSL_LIKELY(len > Vec3(0))) {
        auto invlen = Vec3(1) / len;
        auto nax = ax * invlen;
        auto nay = ay * invlen;
        auto naz = az * invlen;
